(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allTrue, item, index) {
      if(iterator){
        return allTrue && Boolean(iterator(item, index));
      } else{
        return allTrue && item;
      }
    }, true);
  ...
  _.some = function(collection, iterator) {
    ...
    //  some is kind of like an anti-every
    // Every will false whenever it finds false
    // Some will true whenever it finds true
  ...
  _.shuffle = function(array) {
    ...
    for( var i = answer.length-1; i >= 0; i-- ) {
      ...
      holder = answer[i];
      answer[i] = answer[randomPos];
      answer[randomPos] = holder;
    }
  ...
  _.flatten = function(nestedArray, result) {
    var answer = result === undefined ? [] : result;
    
    return _.reduce(nestedArray, function(accum, value,index){
      if(!Array.isArray(value)) {
        accum.push(value);
        return accum; 
      } else {
        return _.flatten(value,accum);
      }
    }, answer);
  };
